...
function setPlayerSelects(){for(let i=1;i<=10;i++){$(`#playerSelect${i}`).select2({data:[{id:'default',text:'Игрок не выбран',selected:!0,},],theme:'bootstrap-5',}).on('select2:select',function(e){console.log({pos:Number(e.currentTarget.id.replace('playerSelect','')),player:e.params.data.id==='default'?{name:'',image:''}:{name:e.params.data.text,image:e.params.data.id,},});playerChannel.postMessage({pos:Number(e.currentTarget.id.replace('playerSelect','')),player:e.params.data.id==='default'?{name:'',image:''}:{name:e.params.data.text,image:e.params.data.id,},})})}}
